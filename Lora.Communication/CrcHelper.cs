...
{
    public static uint Crc32(byte[] data, uint crcInitial)
    {
        ...
        else if(Aes.Arm64.IsSupported)
        {
            ...
            while (cbSize >= 8)
            {
                crc2 = System.Runtime.Intrinsics.Arm.Crc32.Arm64.ComputeCrc32(crc2, BitConverter.ToUInt64(data, ibStart));
                ibStart += 8;
                cbSize -= 8;
            }
    ...
    public static uint Crc32Aligned(byte[] data, uint initial = 0)
    {
        if (data == null) throw new ArgumentNullException(nameof(data));
        ...
        if (Sse42.X64.IsSupported)
        {
            while (cbSize >= 4)
            {
                crc = Sse42.X64.Crc32(crc, BitConverter.ToUInt32(data, ibStart));
                ibStart += 4;
                cbSize -= 4;
            }
